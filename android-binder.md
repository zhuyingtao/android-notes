...
- binder_init(): 注册misc设备
- binder_open(): 打开 binder驱动设备
- binder_mmap(): 映射内核虚拟地址空间
- binder_ioctl(): 收发 IPC 数据和 IPC reply 数据

binder_mmap() 是 binder IPC 通信效率高的核心机制所在
...
ServiceManager是Binder IPC通信过程中的守护进程，本身也是一个Binder服务。ServiceManager本身工作相对简单，其功能：查询和注册服务。 其涉及到的源码：
...
- **IInterface** : IInterface 代表的就是 Server 对象具备什么样的能力（能提供哪些方法），AIDL 文件中定义的接口默认会继承这个接口）
- **Binder** : Java 层的 Binder 类，代表的其实就是 Binder 本地对象。BinderProxy 类是 Binder 类的一个内部类，它代表远程进程的 Binder 对象的本地代理；这两个类都继承自 IBinder, 因而都具有跨进程传输的能力；实际上，在跨越进程的时候，Binder 驱动会自动完成这两个对象的转换。
- **Stub** : AIDL 的时候，编译工具会给我们生成一个名为 Stub 的静态内部类。这个类继承了 Binder, 说明它是一个 Binder 本地对象，它实现了我们自定义的接口（.aidl），表明它具有 Server 承诺给 Client 的能力；Stub 是一个抽象类，具体的自定义接口的相关实现需要开发者自己实现。
- **Proxy** : AIDL的时候，编译工具还会给我们生成一个名为 Proxy 的静态内部类。这个类同样实现了我们自定义的接口（.aidl），并且具体实现了相关方法。这个类里用组合方式持有 BinderProxy 的引用，直接对 BinderProxy 实例进行操作。所以 Proxy 可以近似理解成 BinderProxy 的子类，只不过实际上是做了一层封装。

为什么要分 Stub 和 Proxy 呢？其实它们分别适用于本地调用和远程调用两种情况。如果 server 和 client 是同一进程，则直接用 Stub，如果不是同一进程，那就需要使用 Proxy。

这几个类的继承关系可以用下图描述：

![img](https://upload-images.jianshu.io/upload_images/1507403-90e51bfe85a79136.png)
